...
@router.post(
    "/extract", response_model=KeyRequirementsResponse, status_code=status.HTTP_200_OK
)
async def extract_requirements(
    ...
    try:
        ...
        requirements = await extract_key_requirements(
            job_description_id=request.job_description_id, db=db
        )
...
async def extract_requirements_from_content(
    ...
    try:
        ...
        logfire.error(
            ...
            content_length=len(request.job_description_content)
            if request.job_description_content
            else 0,
            error=str(e),
            error_type=type(e).__name__,
            traceback=logfire.format_exception(e),
