...
async def generate_cover_letter_endpoint(
    cover_letter_request: CoverLetterRequest, db: Session = Depends(get_db)
):
    """
    ...
    resume = (
        db.query(Resume).filter(Resume.id == cover_letter_request.resume_id).first()
    )
    if not resume:
    ...
    resume_version = (
        ...
        .filter(ResumeVersion.resume_id == cover_letter_request.resume_id)
    ...
    job = (
        ...
        .filter(JobDescription.id == cover_letter_request.job_description_id)
        .first()
    )
    if not job:
