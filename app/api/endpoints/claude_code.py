...
PROMPT_TEMPLATE_PATH = os.path.join(
    ...
    "claude_code_resume_prompt.md",
)
WORKING_DIR = os.environ.get(
    "CLAUDE_CODE_WORKING_DIR", os.path.join(tempfile.gettempdir(), "claude_code_resume")
)
...
def read_file(file_path: str) -> str:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error reading output file: {str(e)}"
        )
...
async def customize_resume_async(
    ...
    try:
        ...
        if task_id:
            ...
            if not task:
                logger.info(
                    f"Creating new progress tracker task with provided ID: {task_id}"
                )
                task = progress_tracker.create_task()
...
async def get_customize_status(
    ...
    if include_logs:
        ...
        logs = full_logs[-max_logs:] if len(full_logs) > max_logs else full_logs
...
async def get_customize_logs(task_id: str, db: Session = Depends(get_db)):
    """
...
async def stream_customize_logs(
    ...
    max_duration: Optional[int] = Query(
        3600, ge=10, le=86400, description="Maximum stream duration in seconds"
    ),
    format: Optional[str] = Query(
        "sse",
        description="Output format: 'sse' for Server-Sent Events or 'json' for direct JSON streaming",
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    if format.lower() == "sse":
        ...
        async def sse_generator():
            ...
            yield (
                "data: "
                + json.dumps({"task_id": task_id, "logs": logs, "status": task.status})
                + "\n\n"
            )

            # Stream logs as they arrive using the enhanced streamer
            try:
                async for log in log_streamer.stream_logs(
                    task_id, timeout=max_duration
                ):
                    ...
                    if isinstance(log, dict):
                        ...
                        yield (
                            "data: "
                            + json.dumps({"task_id": task_id, "new_log": log})
                            + "\n\n"
                        )
        ...
        async def json_generator():
            ...
            try:
                ...
                async for log in log_streamer.stream_logs(
                    ...
                    if isinstance(log, dict):
                        yield json.dumps(
                            {"task_id": task_id, "log": log, "type": "log"}
                        )
                    else:
                        # Plain text log
                        yield json.dumps(
                ...
                # Final state
                yield json.dumps(
                    {"task_id": task_id, "status": task.status, "type": "complete"}
                )
...
@router.get(
    "/customize-resume/result/{task_id}", response_model=CustomizedResumeResponse
)
async def get_customize_result(task_id: str, db: Session = Depends(get_db)):
    """
