...
def select_model_for_task(
    ...
    for model_name, config in available_models.items():
        ...
        if all(cap in config["capabilities"] for cap in requirements["required_capabilities"]):
    ...
    if not capable_models:
        print(f"WARNING: No models meet all capability requirements for {task_name}, using any available")
        try:
            logfire.info(
    ...
    if preferred_provider and preferred_provider != ModelProvider.ANY:
        provider_models = {
            name: config for name, config in capable_models.items() 
        ...
        if provider_models:
            ...
            print(f"WARNING: Preferred provider {preferred_provider.value} has no suitable models, falling back to others")
            try:
                logfire.info(
    ...
    if content_length:
        ...
        if sufficient_models:
            ...
            print(f"WARNING: No models have sufficient context length for content of {content_length} tokens, using available")
            try:
                logfire.info(
                    "No models have sufficient context length, using available models",
                    required_length=content_length,
                    max_available=max(config["max_tokens"] for config in capable_models.values()),
                    level="warning"
                )
            except Exception:
                pass
    ...
    for model_name, config in capable_models.items():
        ...
        if (ModelCapability.THINKING in config["capabilities"] and 
            (task_name.startswith("resume_evaluation") or 
             task_name.startswith("optimization") or
             task_name.startswith("feedback"))):
            score += 20.0
        
        # Cost factor - penalize higher cost models based on budget sensitivity
        cost_penalty = (config["cost_per_1k_input"] + config["cost_per_1k_output"] * 3) * 1000 * budget_factor
        score -= cost_penalty
    ...
    logfire.info(
        ...
        available_providers=[config["provider"].value for name, config in available_models.items()],
        task_requirements=requirements,
        capabilities=selected_config["capabilities"],
        top_scores=dict(sorted(model_scores.items(), key=lambda x: x[1], reverse=True)[:3]),
...
def get_fallback_chain(
    ...
    for model_name, config in available_models.items():
        if (model_name != primary_model and 
            config["provider"] == primary_provider and
            config["tier"] == primary_tier):
    ...
    for model_name, config in available_models.items():
        if (model_name != primary_model and 
            config["provider"] != primary_provider and
            config["tier"] == primary_tier and
            model_name not in fallbacks):
            fallbacks.append(model_name)
    ...
    for model_name, config in available_models.items():
        if (model_name != primary_model and 
            config["provider"] == primary_provider and
            config["tier"] == requirements["fallback_tier"] and
            model_name not in fallbacks):
            fallbacks.append(model_name)
    ...
    for model_name, config in available_models.items():
        if (model_name != primary_model and 
            model_name not in fallbacks and
            all(cap in config["capabilities"] for cap in requirements["required_capabilities"])):
            fallbacks.append(model_name)
...
def get_model_config_for_task(
    ...
    logfire.info(
        "Generated complete model configuration",
        task=task_name,
        model=model_name,
