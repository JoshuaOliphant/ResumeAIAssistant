...
def test_generate_cover_letter(resume_id, job_id):
    ...
    response = requests.post(f"{BASE_URL}/cover-letter/generate", json=cover_letter_data)
...
def run_ai_tests(resume, job):
    ...
    if not (ats_result and custom_result and cover_letter):
        print("AI-powered tests failed. Please check if Anthropic API is properly configured.")
        return False
    ...
    if isinstance(basic_results, tuple) and len(basic_results) == 3:
        ...
        if success:
            ...
            if ai_success:
                ...
                print("AI-powered tests failed. The application might still work without AI features.")
    else:
